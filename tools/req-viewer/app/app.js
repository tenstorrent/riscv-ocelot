...
const IMPL_COPY = {
  nlhdl: {
    what: 'NL_HDL source',
    where: 'src/main/nlhdl/**/*.nlhdl.sv',
    how: 'written by <code>/nlhdl gen-nlhdl</code>, module map in <code>src/main/nlhdl/hierarchy.yaml</code>',
  },
  rtl: {
    what: 'generated RTL',
    where: 'the SystemVerilog emitted from the NL_HDL spec',
    how: 'produced by <code>/nlhdl gen-rtl</code>',
  },
};
...
function fillImpl(bodySel, subSel, req, kind) {
  ...
  for (const hit of hits) {
    ...
    const head2 = el('div');
    head2.appendChild(el('span', 'path', `${hit.file}:${hit.line}`));
    card.appendChild(head2);
    card.appendChild(el('pre', null, hit.snippet));
