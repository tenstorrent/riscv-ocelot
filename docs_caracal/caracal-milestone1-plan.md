...
4. **Parameters.** Defaults: `VLEN = 256`, `ELEN = 64`, `numVecPhysRegisters = 96`, `numVlPhysRegisters = 64`. Everything else parameterized (`numVecLoadQueueEntries`, `numVecStoreQueueEntries`, issue-queue widths/entries, LCB depth, SSI/US queue depths, `dcacheArbiterMode`, …). **No cracker parameters** — the atomic mapper removes frontend cracking, so any `crackerWidth`/`numDecodeToCrackerBufferEntries` left from earlier drafts are removed or repurposed (the mapper's group allocation is `coreWidth*8` wide). The default tier under test is `WithNSmallBooms ++ WithVector`, but the implementation must elaborate cleanly at Medium and Large widths too.
5. **No frontend cracking; atomic group rename; OoO across queues.** A vector instruction is a **single `OP.v`** through decode/rename/ROB/issue. The vector mapper renames a whole `LMUL`/`EMUL` group atomically (up to 8 PRNs per `vdest`); element/segment cracking into `nOP.v` happens only in the Vector LS AGEN. All three vector queues are **age-ordered collapsing**; **`IQ_V_ALU` adds a *per-entry* past-PNR eligibility gate** (RoCC-style), so every op handed to the CII is individually non-speculative. This removes **branch**-kill from the CII (the PNR cannot sweep past an unresolved branch) but **not** flush recovery — past-PNR is *not* a commit guarantee (`rob.scala:438-441`), so the CII needs the drain-on-flush contract in `cii.rst` §Flush recovery. `IQ_V_ALU` is deliberately **not** a head-only FIFO: the VPU being in-order requires one-at-a-time execution, not program-order *issue*, and a head-only queue would let a segmented store's coprocessor half block all younger vector arithmetic. Inter-queue ordering is enforced exclusively by the ROB.
...
7. **Precise exceptions.** Vector loads/stores trap with **`vstart = 0` and restart the whole instruction** on an element-level fault. Mid-vector resume (`vstart = k`) is **not** implementable here: the faulting `OP.v` never commits, so its fresh `pvdest` group is reclaimed and elements `0..k-1` are never architecturally visible — resuming at `k` would leave them stale. `fault_elem` is retained only as the element-cursor stop signal and a debug counter. Stores translate/disambiguate all active elements pre-commit and do not write the DCache until ROB-committed.
...
- `src/main/scala/v4/vec/common/VectorParams.scala` —
  ...
  case class VectorParams(
    vLen: Int = 256,
    numVecPhysRegisters: Int = 96,         // vector PRF depth; 32 held by the committed
                                           // map table -> (96-32)/8 = 8 LMUL=8 groups in flight
    ...
    // numVecTmpGroups removed — no free-list headroom is needed for forward progress (rename is
    // in program order, so an op that cannot allocate simply stalls). The requirement is instead
    // that pvdest+pvtmp allocation is all-or-nothing; see Step 3.
...
- `vec/decode/VsetDecode.scala` — classifies the three vset forms and marks routing:
  - **`vsetivli`** (immediate VTYPE + AVL): VCFG updates the `vtype` mirror and computes VL at decode; the value is written to `VL_RF[pvl]` in the **rename cycle** (where `pvl` is allocated — there is no VL-RF index at decode), with **no busy bit set** (`pvl` born ready) and no back-end EU. ROB entry dispatched non-busy.
  ...
  - **`vsetvl`** (register VTYPE + AVL): **`is_unique` *and* `flush_on_commit`**; executes on the integer ALU EU. The EU does **not** write the speculative VCFG mirror — `is_unique` alone does not order it against younger decode (`core.scala:739-740` gates only the unique uop's *own* dispatch; younger uops dispatch one cycle later and would snapshot a stale `vtype` → wrong `EMUL` → mis-sized PRN group). Instead the ROB updates the committed VCFG shadow at commit, and `flush_on_commit` refetches younger uops, which reloads the speculative mirror from that shadow — the existing flush-recovery row, no new path.
  - **All producing forms** set the new **`is_vl_producer`** bit, which is orthogonal to `dst_rtype`: a `vset` writes *two* rename spaces (`pdst` in the INT RF when `rd != x0`, `pvl` in the VL RF always) from one result bus, and with `rd == x0` `dst_rtype` reads `RT_ZERO` while the VL RF is still written.
...
**Files created.**
- `vec/rename/VecMapTable.scala` — 32 entries, **1 PRN per architectural vreg**. An `EMUL`-wide group read returns up to 8 member PRNs per source (`pvs1[8]`/`pvs2[8]`/`pvs3[8]`) + the stale group, correct under arbitrary fragmentation. **No LMUL Tag Table and no whole-group checkers** — deleted: a per-ARN map table is always whole-group correct, so the structure gated nothing (it fed only a perf counter), its test mislabelled 8 independent `LMUL=1` writes as fragmented, and it added a comparator tree to the single-cycle rename critical path. Use a per-ARN "last-write `EMUL`" counter if the observability is still wanted. Group-aware intra-bundle **reg-src / stale bypass** (per-member: forward an older same-bundle lane's freshly-allocated PRN for overlapping members).
- `vec/rename/VecFreeList.scala` — `SelectFirstN`, `allocWidth = coreWidth*8`, **non-contiguous** `EMUL`-group allocation; `deallocWidth = commitWidth*8` (free the whole stale group at commit); `br_alloc_lists` reclaim. **Allocation of `pvdest`+`pvtmp` for a shared op is all-or-nothing** — never take a partial group and wait for the rest, since held PRNs cannot be freed until commit. No free-list headroom reservation is needed for forward progress: rename is in program order, so an op that cannot allocate simply stalls while older ops commit and free (`numVecTmpGroups` is therefore redundant and should be dropped unless it guards something else).
...
- `vec/issue/VecIssueUnit.scala` — all three vector queues wrap `IssueUnitCollapsing` (age-ordered) for the insert/grant widths; **`IQ_V_ALU` adds a per-entry past-PNR eligibility gate** (`is_older(rob_idx, rob.io.rob_pnr_idx)` ANDed into each slot's `request`), RoCC-style. Wire `rob_pnr_idx` into `IQ_V_ALU`. **Not** head-only: program-order *issue* is unnecessary once rename has resolved dependences (the VPU needs one-at-a-time execution, not program order; vtype/vl/vstart/vxrm ride the per-instruction issue packet, and `vxsat`/`fflags` accrue at commit in ROB order). Head-only would let a segmented store's coprocessor half — blocked on its own LSU half's 256-element translation — stall every younger vector arithmetic op. Squashed entries are dropped before issue; ops already accepted by the CII are handled by drain-on-flush (M2 Step A9).
- **Vector store slot DGEN gate:** `dgen_operand := Mux(uop.is_shared, uop.pvtmp, uop.pvs3)`. Gating unconditionally on `pvs3` is **wrong for a segmented store** — `pvs3` is the *coprocessor* half's source and is ready long before the transpose runs, so DGEN would read `pvtmp` before it is written and store garbage.
...
**Files created.**
- `vec/regfile/VecRegFile.scala` — `numVecPhysRegisters=96`, `VLEN=256`, **9 read / 3 write ports**, banked 4×64b, single-cycle read with write-forwarding. Ports are **statically partitioned, 0-based** (canonical table in `midcore.rst` §Port assignment): Load/LCB `R0` index, `R1` mask, **`R2` `stale_pvdest`**, `W0`–`W1`; Store `R3` data, `R4` mask/index, no writes; CoProc `R5`–`R8` (4 pull lanes), `W2` (Milestone 2). Both CoProc figures are **derived from the frozen SV contract** `tt_cii_caracal_pkg.svh`, not chosen: `CII_NUM_SRC_REQ = 4` (the VPU wrapper returns member `k` of `{VS1,VS2,VS3,VM}` in one dat beat, so all four lanes can be live at once) and `CII_NUM_DST_WB = 1` (flagged must-match-the-`tt_cii_interface`-default — the relay's `type(wb_data)` resolves against it). Earlier drafts said 7R/4W on the reasoning that a pull interface "needs 2 reads, not 4"; that was wrong in both directions — 4 reads, 1 write. Note `tt_cii_caracal_pkg.svh` contradicts itself here (the inline comment on `CII_NUM_SRC_REQ = 4` still reads "2 VRF read ports for CII (5,6)"); the **value** is authoritative. `R2` is **new**: the LCB pre-loads each assembly entry from `stale_pvdest` for `vta=0`/`vma=0` loads, without which a short-`vl` undisturbed load writes junk into its tail. `pvtmp` groups live here (no separate temp file). Static partitioning means the CII never stalls on a write port — required, since its Writeback channel is credit-metered with no back-pressure line.
...
**Files modified.**
- `exu/execution-units/*` (integer ALU) — for a `vset` uop: read `rs1` (and `rs2` for `vsetvl`) from the integer RF/bypass, compute `VL = min(rs1, VLMAX)` (and VTYPE for `vsetvl`), and on writeback **target the VL RF** + drive the **VL wakeup network**. The EU does **not** write the speculative VCFG mirror (see Step 2 — `vsetvl` carries `flush_on_commit` and recovers the mirror from the committed shadow instead). When `rd != x0`, also write `rd` normally: rename set **both** busy bits (INT for `pdst`, VL for `pvl`) and commit runs **both** free paths (stale `pdst`, outgoing committed VL pointer), gated on `is_vl_producer` / `dst_rtype` respectively. Bit-identical to BOOM when `usingRVV=false`.
...
- **VRF banking / port tuning** beyond 9R/3W.
...
| Atomic group rename free-list pressure | `SelectFirstN allocWidth=coreWidth*8`; **all-or-nothing** `pvdest`+`pvtmp` allocation for a shared op (no headroom reservation — in-order rename makes stalling safe) |
