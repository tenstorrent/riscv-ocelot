...
class VecStoreWalker(implicit p: Parameters)
extends BoomModule with VecLsConstants {
  ...
  val io = IO(new Bundle {
    ...
    val vdb_data = new VecDgenDataIF
    // status signal
    val gen_active = Output(Bool())
    // store packet
    val store_packet = DecoupledIO(new VecStoreNop)
  })
  ...
  io.store_packet.bits.pvs3        := 0.U
  io.store_packet.bits.pvs3_member := 0.U
  ...
  switch (state) {
    ...
    is (State.IDLE) {
      when (io.start.fire) {
        ...
        current_ctr     := Mux(is_index, vstart, 0.U) // will be adjusted in vstart handling
        ...
        vstart_addr_done_q    := is_index // dont want to do this for indexed (so set to complete immediately)
