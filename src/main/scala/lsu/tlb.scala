...
class NBDTLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p) {
  ...
  val io = IO(new Bundle {
    val req = Flipped(Vec(memWidth+1, Decoupled(new TLBReq(lgMaxSize))))
    val miss_rdy = Output(Bool())
    val resp = Output(Vec(memWidth+1, new TLBResp))
  ...
  def widthMap[T <: Data](f: Int => T) = VecInit((0 until memWidth+1).map(f))
  ...
  val pmp = Seq.fill(memWidth+1) { Module(new PMPChecker(lgMaxSize)) }
