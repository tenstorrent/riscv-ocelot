...
class LoadPacker(val VLEN: Int) extends Module {
  ...
  val io = IO(new Bundle {
    val start = new Bundle {
      ...
      val stride_enc  = Input(UInt(64.W))
      val seg_enc     = Input(UInt(64.W))
  ...
  val stride_enc = RegInit(0.U(64.W))
  val seg_enc    = RegInit(0.U(64.W))
  ...
  val el_mask_width     = log2Ceil(vlen_bytes) - eew_enc // eq to log2Ceil(vlen_bytes >> eew_enc)
  ...
  def split_ctr(ctr: UInt): CTR_INFO = {
    ...
    ctr_info.seg_id     = (ctr >> seg_mask_off)     & ((1 << seg_mask_width) - 1).U
    ctr_info.stride_id  = (ctr >> stride_mask_off)  & ((1 << stride_mask_width) - 1).U
    ctr_info.el_id      = (ctr >> el_mask_off)      & ((1 << el_mask_width) - 1).U
    ctr_info.v_group_id = (ctr >> v_group_mask_off) & ((1 << v_group_mask_width) - 1).U
  }

  // ===== Packing Constraints =====
  // VLEN / EEW
  val vlen_constraint = (vlen_bytes >> eew_enc)
  // {next group_id (group_id+1), first everything else} - CTR 
  val vreg_constraint = (EEW_CTR & ~((1 << v_group_mask_off) - 1)) + (1 << v_group_mask_off) - EEW_CTR
  // {last group_id (EMUL), last el_id (final_el_id), first stride_off ('0'), last seg_id ('1')} - CTR + 1
  val final_el_id     = vl & ((1 << el_mask_width) - 1) // final el_id = vl % ids_per_vlen
  val vl_constraint   = ((emul_enc << v_group_mask_off) | (final_el_id << el_mask_off) | ((1 << stride_mask_off) - 1)) - EEW_CTR + 1

  // ===== Advance CTR based on constraints =====
  val ctr_inc_val = Seq(vlen_constraint, vreg_constraint, vl_constraint).min
  val ctr_inc_case = PriorityMux(Seq( // VL must have high priority
    (ctr_inc === vl_constraint)   -> CTR_INC_CASE.VL,
    (ctr_inc === vreg_constraint) -> CTR_INC_CASE.VREG,
    (ctr_inc === vlen_constraint) -> CTR_INC_CASE.VLEN
  ), CTR_INC_CASE.NONE)

  // ===== look past unwanted stride offsets =====
  val el_off = Mux(
    (split_ctr(EEW_CTR).stride_id =/= 0.U),
    (1 << v_group_mask_off) - (EEW_CTR & ((1 << v_group_mask_off) - 1)), 0.U)
  ...
  // packet info
  io.load_packet.addr   = base_addr + (1 << (EEW_CTR + eew_enc + 3)) // +3 since eew is in bytes
  ...
  io.load_packet.el_count = 0 // io.load_packet.el_count = <use ctr_inc_val and case> TODO: need to figure this out
