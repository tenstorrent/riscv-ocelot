...
<|begin_module|>
  ...
  ---- 0a. `lsuWidth > 1` LANE DISCIPLINE — READ THIS BEFORE ANY PER-LANE CODE ----

  A FAMILY-WIDE RULE, replacing two point fixes. Architects E and F independently
  found duals of ONE pathology at `lsuWidth = 2`: E, N per-lane events collapsed
  by a `PriorityMux` so only one survived; F, one duty pinned to lane 0 while the
  state it depended on was mutated by lane 1. Same disease, opposite symptoms —
  a lost event and a lost obligation.

  (a) ANY SIGNAL PER-LANE IN ORIGIN MUST BE REDUCED WITH A MULTIPLICITY-PRESERVING
      OPERATOR: OR for flags, `PopCount` for counts, per-lane fan-out for effects.
      NEVER `PriorityMux`, `PriorityEncoder`, or a lane-0 constant. Those three
      silently answer "one of them" to a question whose answer is "how many" or
      "which ones".

  (b) ANY PER-LANE DUTY CONDITIONED ON SHARED MUTABLE STATE MUST OWN THAT STATE.
      Either the state advances only when the duty's own lane fires, or the duty
      is LATCHED UNTIL DISCHARGED. A duty that reads shared state it does not
      control is a duty another lane can delete.

  (c) A `(i == 0).B` TERM INSIDE A PER-LANE LOOP IS A REVIEW FLAG. It means lane 0
      holds a duty no sibling can discharge — so lane 0 must not be able to lose
      its turn to a sibling. `VecDcacheArbiter` has INDEPENDENT per-lane `ready`
      (measured: lane 1 granted while lane 0 refused in the same cycle), so at
      this seam lane 0 CAN lose its turn, and the term is wrong unless something
      else preserves the duty.

  ⚠ THE TWO ASSERTIONS FROM THIS RULE — `laneZeroDutyPreserved` and
  `laneReduceNoLoss` — ARE SILENT AT `lsuWidth = 1`, WHICH IS EXACTLY WHY NEITHER
  DEFECT WAS EVER REACHED. Any bring-up or formal run done only at `lsuWidth = 1`
  provides NO evidence about either property. Treat a `lsuWidth = 1` pass as
  untested for this rule rather than as a clean result.

  ---- 0b. ABSENCE MUST PROPAGATE — THE THIRD FAMILY, AND THE MOST EXPENSIVE ----

  **When a module declines to produce a thing — a range, a completion event, a
  renamed operand slot — every downstream module that could consume it must be
  told, and must be able to tell. A downstream default that assumes presence
  (`read_vrf` true, a completion that only a beat can generate, a reduce over
  lanes) converts an upstream CORRECT decision into silent corruption or a hang,
  and the resulting failure is always attributed to the wrong module. Where a bit
  already records the decision (`v_uses_vs*`, `vl_zero`, per-lane valids), the
  consumer reads that bit; it does not re-derive the answer and it does not
  default.**

  This is not an abstraction after the fact. Three defects of this exact shape
  were found in one campaign, in three different subsystems, and in every one of
  them the module that DECIDED was right and the module that ASSUMED was wrong:

    | upstream decision (correct) | downstream assumption | symptom, and where it appeared |
    |---|---|---|
    | `VDecode`: `vid.v` has no vs2, so do not rename it | `VecCiiTagTable`: `read_vrf := true.B`, a literal per slot | VRF prn 0 served as a real operand; looked like a VPU or a rename bug |
    | `VecRangeAgen`: a vl=0 store accesses no memory, so elide it | `VecLsu`: the completion is generated by a beat, and there are no beats | ROB head wedged forever; looked like a structural-full stall |
    | a lane: I did not fire, so I advanced nothing | the LCAM duty: lane 0 holds it, and the cursor is shared | ordering publication silently dropped; looked like a data-forwarding bug |

  THE COMMON COST IS MISATTRIBUTION, NOT JUST THE BUG. In all three the symptom
  surfaced in a module that had done nothing wrong — a TLMonitor complaining
  about a diplomatic parameter, a core hang assertion, a cosim mismatch on an
  unrelated instruction — because absence travels no further than the first
  consumer that assumes presence, and everything after that point is reasoning
  from a value that should not exist.

  THE TEST TO APPLY when writing a consumer: *if my producer had decided NOT to
  produce this, would I be able to tell?* If the answer is "the field would be
  zero" or "the valid would be low next cycle" or "it cannot happen for this op
  class", that is a default, not a signal — and a default is how this family
  reproduces. Ask for the bit, and if no bit exists, that is the defect.

  ---- 0h. PER-RANGE PROGRESS STATE MUST BE RESET ON ABANDONMENT, AND
           "ABANDONED" IS NOT THE SAME EVENT AS "COMPLETED" ----

  **Every register that records how far through a range the machine has got must be
  cleared by BOTH exits: completion AND abandonment. A range that is branch-killed
  or flushed never reaches its completion event, so a reset written only there
  leaves the state for the NEXT range to inherit.** Two measured instances, opposite
  sides of the LSU, one shape:

  - the load unit-stride CURSOR was cleared only at the load's retire. A killed
    range left it mid-way, the replacement range resumed from there, and every
    member below the inherited position was never fetched at all — the coalescing
    buffer's entry for member 0 sat at 0 of 32 bytes until its watchdog fired
    (`ms4p6_vle32_2`: head `rob_idx` moved 39 → 37 with the cursor still at element
    10 of 16).
  - the store unit-stride WRITE-PASS flag was cleared only at `us_pop`. An abandoned
    range left it SET, so the next range ran in write-pass mode and skipped pass 1 —
    which is where the data enqueue and the LCAM publication live. The store data
    queue's `filled` mask stayed zero for an entire run, which then livelocked an
    unrelated scalar load replaying against a store that could never forward.

  ⇒ THE CHECK IS MECHANICAL AND MUST BE RUN WHENEVER A PROGRESS REGISTER IS ADDED:
  enumerate every per-range progress register, and for each name the event that
  clears it and ask whether an abandoned range reaches that event. Current
  enumeration:
    - load US cursor — completion (retire) + abandonment (flush/kill). CORRECT.
    - store US cursor — `us_pop`, pass-1-complete promotion, + abandonment. CORRECT.
    - store US write-pass — `us_pop` + abandonment, the latter GATED on the entry
      not having committed. CORRECT, and the gate is load-bearing: a committed
      store's write pass is POST-COMMIT and must never be abandoned by a flush, or
      the fix trades lost bytes for a lost committed store, which is worse.
    - load/store LCAM done-records — flush/kill, already present (the store's also
      gated on the write pass, for the same post-commit reason).
    - **load US drain pointer — WAS the unchecked row above, and it was BROKEN.
      Now fixed.** The assumption it rested on — "the queue's squash rewinds the
      tail to the matching position" — is true of the TAIL and false of the
      POINTER, because nothing rewound the pointer at all. Measured on
      `ms4_vle64` and `ms4p5_vle64_2`: at the squash the queue clears `filled` and
      later rewinds `tail` 1 → 0, while the pointer is INCREMENTED 0 → 1 by a
      `ldUsRetire` firing in the same cycle. The re-enqueued range lands at index 0,
      the pointer sits at 1, `filled(1)` is 0, so the range NEVER STAGES: no beat is
      ever offered and the coalescing buffer's entry waits out its watchdog. The two
      moved in OPPOSITE directions because only one of them was rewound.
      ⇒ Rewind the pointer from the SAME event and the SAME value the queue uses —
      `q_squash(<this queue>).bits` is the ABSOLUTE new tail — and place it LAST in
      program order so it beats a same-cycle retire increment. The race is REAL, not
      hypothetical: the squash valid is `branch || flush` for every queue with
      nothing excluding a simultaneous pop, and the measurement above IS the
      coincidence — `filled` cleared and the pointer incremented in one cycle.

      ⚠ **THE REWIND IS CONDITIONAL — IT MUST NEVER MOVE THE POINTER FORWARD.**
      `q_squash.bits` is the rolled-back ALLOCATION tail, i.e. the frontier of the
      youngest SURVIVING row, so whenever the STAGED range itself survives the
      squash that value is STRICTLY AHEAD of the drain pointer (at least
      base+count of the staged row). Following it forward abandons a range that is
      mid-flight: its remaining beats are never generated, its LCB group never
      reaches `group_done`, its PRNs stay busy, the vector free list drains and
      rename deadlocks. Measured on `axpy-vector` (LMUL=8/e64, Mega): a mispredict
      at rob=110 pushed the pointer 1 → 2 while rob=105 still owed 25 of its 32
      elements, and `VecLoadCoalescingBuffer`'s watchdog fired on the short member
      ~4k cycles later. So the unconditional rewind fixed the backward case and
      introduced a forward one.

      ⚠ **AND THE GUARD THAT FIXED IT — `!ldUsStagedValid || kill(<ld_beat>)` —
      WAS ITSELF WRONG, FOR THE THIRD TIME ON THIS ONE REGISTER.** It reads "the
      pointer may follow the tail whenever nothing is staged", i.e. it uses
      *nothing is staged right now* as a proxy for *the pointer is already past the
      surviving region*. Those are different statements, and the gap between them
      is an ordinary cycle: a range that SURVIVES the squash is un-staged whenever
      its slot has not been FILLED yet — the reservation is claimed at dispatch,
      the entry is written when the range AGEN reaches it, and a branch resolving
      in that window is routine. The rewind then jumps the pointer FORWARD over
      every surviving unconsumed range, and none of them is ever drained again.
      Measured on `conv1d-vector` (MegaBoom): ranges at slots 10 (`rob 28`) and 11
      (`rob 32`) both survived a branch at `ldq 37`; the reservation correctly
      rolled the tail back to 12, ABOVE both of them, and the drain pointer was set
      to 12 — skipping both. Zero beats followed, and it surfaced as
      `lcbStallWatchdog` on an entry belonging to one of them.

      ⇒ **THE CONDITION IS AN ORDER COMPARISON, AND ONLY AN ORDER COMPARISON:
      rewind iff the pointer is AT OR BEYOND the new tail.** Both the pointer and
      the new tail lie in the queue's `head … tail` region, so distance-from-the-
      PRE-squash-tail (`io.resv.tail`, the value both were formed against) is a
      total order over them; rewind on
      `dist(ldUsDrainPtr) <= dist(newTail)` and never otherwise. That subsumes both
      earlier arms rather than sitting beside them: a full flush rolls the tail back
      to the head, where every pointer is at-or-beyond, so the flush case is
      unchanged and the drained-empty case (pointer == tail, rollback moves the tail
      behind it) still follows the tail backward — the original defect stays fixed.
      A killed staged range is younger than the branch and therefore sits at or
      above the new tail, so it rewinds without a separate `kill` term. Do NOT
      re-add one: `kill` says nothing about where the pointer is, and a forward jump
      is exactly what must never happen.
      (`VecRangeEntry` carries no `br_mask`, so `IsKilledByBranch` on the
      synthesised kill uop degrades to `rob_flush` — which is why the old guard's
      `kill` arm bought nothing that the order comparison does not already give.)
      THE PATTERN ACROSS ALL THREE REVISIONS: each guard was written from the state
      the author had in mind (the staged entry) rather than from the property that
      actually has to hold (the pointer never moves forward). State the property,
      then test the property.

      ⚠ AND THE SHARPEST LESSON IN THIS WHOLE SECTION: **the rule was already
      written at the very line that had the bug.** The cursor reset's own comment
      reads *"ABANDONED is not COMPLETED — `ldUsRetire` is a completion event"* —
      and it sits on the same statement that increments the drain pointer:
      `when (ldUsRetire) { ldUsCursor := 0.U; ldUsDrainPtr := ldUsDrainPtr + 1.U }`.
      The §0h reasoning was correct, present, and applied by hand to the register
      the author was thinking about, while the second register **two tokens away on
      the same line** was left monotonic. **A rule applied to the register you have
      in mind is not a rule applied to the class.** That is the entire argument for
      the enumeration: not to restate the rule, but to force every member of the
      class past it — including the ones already in front of you.
    - **store US drain pointer, and the three SSI base pointers (load, store pass 1,
      store pass 2) — STILL UNCHECKED, and deliberately NOT fixed on suspicion.**
      They are structurally identical (monotonic, advanced only by their own
      completion event, no rewind), so the same defect is plausible in all four. But
      ⚠ **"STRUCTURALLY IDENTICAL" IS A REASON TO LOOK, NOT A REASON TO EDIT — and
      this pulls the OPPOSITE way from the enumeration rule above, deliberately.**
      The enumeration exists to make you FIND every member of a class; it does not
      license fixing the ones you have not observed. The discriminator is evidence,
      not shape: the width family's sites were **CONFIRMED** instances left unfixed,
      which is why leaving them was wrong; these four are **UNOBSERVED** instances
      that merely share a form. **Fixing an unobserved defect is how you introduce
      one** — a spurious rewind of a pointer that never needed one is a week to find.
      ⇒ **Enumerate to find; measure to fix.**
      no waveform in hand exercises them: in every store-bearing run pinned so far
      (`ms5p1_vse64_m`, `ms5p3_stride_m`) the store US queue's tail is MONOTONIC —
      no squash rewind ever occurs — and the SSI base pointers show no activity at
      all. **Confirming a defect requires seeing the misalignment, not inferring it
      from a matching shape.** Left unchecked, in these words, exactly as the load
      pointer was — that entry is what led to this fix.

  Note the reset condition is written as `rob_flush || kill(n)` even where the kill
  term currently reduces to the flush (the beat kill uops carry no branch mask yet).
  That is deliberate: when the branch mask is populated the reset becomes correct
  for mispredicts with no further edit.
  ...
  ===> THE LAST SENTENCE OF THIS PARAGRAPH MANDATED A BUG AND IS RETRACTED. It
  read: "`data_base` is still sampled at the AGEN pulse, where its `resv_lookup`
  lane is live, and held for the later grant." The clause after the comma is
  FALSE FOR THE RANGE CLASS, and it is stated as a settled fact rather than as
  the assumption it was.

  `resv_resp` is a combinational echo GATED BY ITS OWN LANE'S VALID, returning
  ZERO — not held, not don't-care — when that lane is dead
  (`VecQueueReservation.nlhdl`, the resv_resp paragraph). The two store lanes are
  NOT on the same clock:

    | lane | driver          | its `resv_lookup.valid` is high on |
    |------|-----------------|------------------------------------|
    | 1    | `st_elem_agen`  | the AGEN pulse (`stOpPulse`)       |
    | 3    | `st_range_agen` | `stMaskFire`                       |

  and `stMaskFire` equals `stOpPulse` only for an UNMASKED op — `VecMaskStream`
  raises `us_mask.valid` on the op pulse solely via the `elided_load` path. For a
  MASKED unit-stride store it is ONE CYCLE LATER, so sampling lane 3 at the AGEN
  pulse reads a dead lane and latches 0. Measured: at 3763000 the op is valid
  with `resv_lookup_3_valid=0`/`base=0`; one clock later all of
  `us_mask.valid`/`resv_lookup_3_valid`/`base` go 1/1/**1** and
  `range_bits_us_data_base=1` — while the register latches **0**, in the same
  instant the correct value is on the wire it should have read.

  CORRECT RULE: **capture PER CLASS, each on ITS OWN lane's live cycle** — SSI on
  `stOpPulse && !stIsRangeClass` from lane 1, RANGE on
  `stMaskFire && stIsRangeClass` from lane 3. The deferral is safe because
  `stIsRangeClass` and `stq_idx` both come from `st_opnd.io.out.bits.uop`, which
  holds until `st_msk.io.done`, and `stStreamerBusy` blocks the next `stPresent`
  until then. It is also free: one cycle, masked ops only, consumed ~six cycles
  later at `dgen.req_base`.

  ---- (k2c) RANGE-AGEN PORT TIMING, STATED GENERALLY BECAUSE ONE SENTENCE IS NOT
  ENOUGH ----

  The paragraph above fixes one reader. This one exists because fixing only that
  reader leaves the trap intact for the next: **`grep resv_lookup` returns exactly
  ONE hit**, so nothing in this spec ever said that the ports of a RANGE agen run
  on a different clock from those of an ELEMENT agen. That omission — not the
  wrong sentence — is what let the two clocks be conflated, and it would let them
  be conflated again.

  THE RULE: every port of `st_range_agen`/`ld_range_agen`, and every signal
  derived from one, is valid on the **MASK-FIRE** clock, because
  `VecRangeAgen.io.req.valid` is tied to `stMaskFire`/`ldMaskFire` and not to the
  op pulse. Every port of `st_elem_agen`/`ld_elem_agen` is valid on the **AGEN
  PULSE**, because `VecElemAgen` drives its `resv_lookup.valid` from `accept`.
  Before sampling ANYTHING keyed off a range agen, ask which of the two clocks it
  is on; if the answer is "the op pulse", it is wrong for every masked op and
  accidentally right for every unmasked one — which is exactly the failure that
  hides in a suite where most stores are unmasked.

  A worked reminder of how quiet this class is: `ms11a4_vse_mask` is a masked US
  store and PASSES, because it is the only vector store in its program, so
  `allocTail` is still 0, the true base IS 0, and the corrupt capture is
  ACCIDENTALLY CORRECT. Passing tests are not evidence about this rule.
  ...
  ===> CLEAR `iw_p1_bypass_hint`/`iw_p2_bypass_hint` ON THE WAY INTO THE ROW.
       They are the one class of field on the granted uop that is TRUE OF A CYCLE
       rather than of the op: the hint means "the producer's integer regfile write
       is still in flight AS OF NOW", it is a wakeup-cycle pulse in VecIssueSlot,
       and `xx_opnd` arms its writeback wait from the hint on the uop it is
       PRESENTED. A row holds a grant for as long as the streamer is busy — tens
       of cycles for one 64-byte group — so a hint replayed out of a row makes
       `xx_opnd` wait for a write that landed long ago. Nothing writes that PRN a
       second time, so the wait is PERMANENT: it surfaces as `ld_opnd`'s
       `wbWaitWatchdog` thousands of cycles later with the whole vector pipe
       stalled behind a lane waiting on a producer that has already retired.
       Measured on `conv1d-vector` (MegaBoom): rob 94 presented with `prs1 = p9`,
       `hint0 = 1`, long after `VecIssueUnit` granted it, and no `int_wb_snoop`
       write to p9 ever followed.
       The FRESH path — the grant presented in its own grant cycle, bypassing the
       row — keeps its hints and still arms; it is the only path on which the hint
       is true. A row is presented at least one cycle later, by which time the
       write is either already in the register file or lands in the read-RESPONSE
       cycle, where `xx_opnd`'s snoop forward covers it. This is a FIELD-SPECIFIC
       exception to "the row holds the granted uop unchanged", and it belongs here
       rather than in `xx_opnd`, which cannot see how long its input was parked.
  ...
  ===> `src_off` IS THE PARKED `byte_en`'s FIRST SET BYTE, NEVER THE PARKED
       `vaddr`'s LOW BITS. VecBeatExpander's LOAD path deliberately WORD-ALIGNS
       the address it requests (`(addr >> beatOffBits) << beatOffBits`) and carries
       the offset within that word in `byte_en` alone, so `vaddr(2,0)` of a parked
       load request is ZERO for every non-corner beat and encodes nothing. Taking
       `src_off` from it silently reads a misaligned beat's bytes from offset 0 of
       the response word — the WRONG bytes, of the right length, into the right
       destination offset, which no length or completion check can see. Measured on
       `conv1d-vector` (MegaBoom, `vl2re32.v v12, (x30)`, `x30 = 0x800032c8`): the
       one beat of the whole 64-byte transfer that was not 8-byte aligned placed
       element 0's bytes at `dst_byte = 4`, and every other beat in the test was
       aligned, so the corruption was exactly four bytes wide out of sixty-four.
       `PriorityEncoder(byte_en)` is correct on BOTH beat shapes and is the only
       form that is: for a `corner` beat the expander sends the RAW address with
       `mem_size = eew`, the D$'s `LoadGen` right-aligns those bytes to bit 0, and
       `byte_en`'s first set byte is 0 to match; for a full-beat access it sends
       `mem_size = beatOffBits`, `LoadGen` returns the word unshifted, and
       `byte_en`'s first set byte is the lane offset the expander put there.
       `nbytes` already reads `byte_en` and not the address, for the same reason.
