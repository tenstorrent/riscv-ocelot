...
<|begin_module|>
  ...
  through `boomParams.vector.get`:
    ...
    `vecELen`, `maxVecVL`, `vecVLSz`, `maxVecMembers`, `ldRespTags`,
    `ldRespTagSz`, `ciiTagBits`, `ssiQueueEntries`, `usQueueEntries`,
    `lcbEntries`, `dcacheArbiterMode`.

  `ldRespTagSz` must be re-exported and not only left on `HasVectorParams`:
  `MicroOp.v_mem_tag` is sized by it, and `MicroOp` is a `BoomBundle` that sees the
  core parameters, not that trait. A derived vector width used by any bundle
  outside the `vec` package needs a forwarder here or it does not resolve.
  ...
  4. HOISTED AT E3: derive `numVecIrfWritePorts = if (usingRVV) 1 else 0`,
     `numVecIrfReadPorts = if (usingRVV) 5 else 0`, and
     `numIrfWritePorts = aluWidth + lsuWidth + 1 + numVecIrfWritePorts` on the trait.
     These lived as local `val`s inside `BoomCore`, which made FOUR specs wrong at
     once: `VecScalarOperandRead`, `VecLsu`, `VecCiiHost` and `VecCiiIssue` all state
     that `numIrfWritePorts` is available "from `HasBoomCoreParameters`", and it was
     not. It cannot be passed down either — `hierarchy.yaml` fixes those modules'
     constructor parameters, and `VecScalarOperandRead`'s is `isStore` alone.
     The first generation attempt therefore RE-DERIVED BoomCore's private formula
     inside the module. That compiles and is even self-checking at the connect point
     (a `Vec` size mismatch is a hard error, not a silent bug), but it is the
     two-copies-of-one-number pattern this project keeps paying for, and the copy
     lives in a file whose whole purpose is to be regenerated by a model that cannot
     see `core.scala`. Same reasoning as (1): the trait is the first scope where all
     the operands exist, so that is where the number belongs.
     `numIrfLogicalReadPorts` STAYS in `BoomCore` — it reads `all_exe_units`, which
     the trait cannot see. The test is whether the operands are in scope, not whether
     the name looks similar.
     Gate (f): unchanged. The formula is identical and `numVecIrfWritePorts` is zero
     when `!usingRVV`, so every vectors-off build computes exactly what it did before.
